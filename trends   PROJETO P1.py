...
def tweet_time(tweet):
    ...
    hora = datetime.today()
    return hora.year, hora.month, hora.day, hora.hour, hora.minute, hora.second #retorna a hora em que o tweet foi publicado
  
def tweet_location(tweet):
    """Return a position (see geo.py) that represents the tweet's location."""
    return make_position(tweet['latitude'], tweet['longitude']) #retorna a localização a função make_position foi escrita no arquvio geo.py
    

def tweet_string(tweet):
    """Return a string representing the tweet."""
    return '"{0}" @ {1}'.format(tweet['text'], tweet_location(tweet)) #função inalterada
...
def extract_words(text):
    ...
    """ 
    # A função devolve uma lista de strings com todas as pontuações removidas. 
    lista = []   
    indice_inicial = 0
    for index, character in enumerate(text): #A função enumarate() retorna um objeto iterável 
        if character not in ascii_letters: 
            if indice_inicial is not None:
                lista.append(text[indice_inicial:index]) 
            indice_inicial = None
        elif indice_inicial is None:
            indice_inicial = index
    if indice_inicial is not None:
        lista.append(text[indice_inicial:])
    return lista
...
def make_sentiment(value):
    ...
    if value ==  -1:
        print(value, 'muito negativo')
    elif value == 1:
        print(value, 'muito positivo')
    elif value == 0:
        print('Sentimento não existe')

def has_sentiment(s):
    """Return whether sentiment s has a value."""
    if make_sentiment(s) == None:
        return False    
    else:
        return True
...
def analyze_tweet_sentiment(tweet):
    ...
    average = make_sentiment(None)  # recebe um tweet e retorna um sentimento 
    tmundo = tweet_words(tweet)
    total = 0
    count = 0
    for x in tmundo:
        sentiment = get_word_sentiment(x)    
...
def find_centroid(polygon):
    ...
    for i in range(len(polygon) - 1):
        total += (latitude(polygon[i]) + latitude(polygon[i + 1])) * ((latitude(polygon[i])*longitude(polygon[i + 1])) - (latitude(polygon[i + 1])*longitude(polygon[i])))
    centroid_latitude = total / (6 * area)

    total = 0

    for i in range(len(polygon) - 1):
        total += (longitude(polygon[i]) + longitude(polygon[i + 1])) * ((latitude(polygon[i])*longitude(polygon[i + 1])) - (latitude(polygon[i + 1])*longitude(polygon[i])))
...
def find_closest_state(tweet, state_centers):
    ...
    for estado, centre_position_of_state in state_centers.items():
        if melhor == None:
            melhor =  geo_distance(centre_position_of_state, tweet_location(tweet))
            EstadoProximo = estado          
            continue
        else:
            distancia = geo_distance(centre_position_of_state, tweet_location(tweet)) 
            if  distance < melhor:
                melhor = distance
                EstadoProximo = estado          
...
def group_tweets_by_state(tweets):
    ...
    """
    TweetsPorEstado = {}
    usa = {n: find_center(s) for n, s in us_states.items()}
    for tweet in tweets:
        ChaveDoEstado = find_closest_state(tweet, usa)
        TweetsPorEstado.setdefault(ChaveDoEstado, []).append(tweet)
    return TweetsPorEstado
...
def most_talkative_state(term):
    ...
    TweetsPorEstado = group_tweets_by_state(tweets)
    ...
    J = None
    MaioriaEstados = None
    for estado, K in talkative_states.items():
        if J == None:
            J = K
            MaioriaEstados = estado
            continue
        else:
            if K > J:
                J = K
                most_talkative_state = estado
    return MaioriaEstados
...
def average_sentiments(tweets_by_state):
    ...
    SentimentosPorEstado = 0
    J = 0

    for estado, tweet_list in tweets_by_state.items():
        for tweet in tweet_list:
            sentimento = analyze_tweet_sentiment(tweet)
            if not ((sentimenot == 0) or (sentimento == None)):
                SentimentosPorEstado += sentimento
                J += 1
        if SentimentosPorEstado != 0:
            averaged_state_sentiments[estado] = (SentimentosPorEstado / J)
        SentimentosPorEstado = 0
        J = 0
